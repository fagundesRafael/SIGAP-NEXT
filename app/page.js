    ...
    <div className="min-h-screen flex items-center justify-center bg-c_deep_black">
      <div className="relative">
        ...
        >
          {/* Cabeça do cadeado */}
          <div 
            className={`relative z-20 transition-all duration-700 ease-in-out mb-[-30px] ${
          ...
          {/* Corpo do cadeado */}
          <div 
            className={`relative z-10 transition-all duration-700 ease-in-out mt-[-30px] ${
              isOpen ? "transform translate-y-3 opacity-80" : ""
            }`}
          >
            <Image 
              src="/sigap_body-padlock.png" 
        ...
        <div 
          ...
          className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-96 h-96 transition-all duration-700 ease-in-out ${
            isOpen && !isTransitioning ? "opacity-100" : "opacity-0 pointer-events-none"
          }`}
        >
          {menuItems.map((item, index) => {
            ...
            const selectedStyle = isTransitioning && isSelected
              ? {
                  left: '50%',
                  top: '50%',
                  opacity: 1,
            ...
            // Estilo para os itens não selecionados durante a transição
            const nonSelectedStyle = isTransitioning && !isSelected
              ? {
                  opacity: 0,
            ...
            return (
              <div 
                ...
                style={{ 
                  ...selectedStyle,
                  ...nonSelectedStyle,
                  transition: isTransitioning ? 'all 0.7s ease-out' : 'all 0.3s ease-out'
                }}
