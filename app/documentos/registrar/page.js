...
export default function RegistrarDocumento() {
  ...
  async function handleSubmit(formData) {
    ...
    try {
      ...
      if (res.ok) {
        setNotificationMessage("Documento criado com sucesso!");
        setShowNotification(true);
        setTimeout(() => router.push("/documentos"), 2000);
      } else {
        const data = await res.json();
        setErrorMsg(data.error || "Erro ao criar documento");
      }
    } catch (error) {
  ...
  return (
    <>
      <DocumentosForm onSubmit={handleSubmit} title="Registrar Novo Documento:" />
