// app/api/outrosobjetos/[id]/route.js
...
export async function GET(request, { params }) {
  try {
    ...
    const record = await OutrosObjetos.findById(id);
    if (!record) {
...
export async function PUT(request, { params }) {
  try {
    ...
    const updatedRecord = await OutrosObjetos.findByIdAndUpdate(id, body, { new: true, runValidators: true });
    if (!updatedRecord) {
      return new Response(JSON.stringify({ error: "Registro não encontrado" }), { status: 404 });
    }
...
export async function DELETE(request, { params }) {
  try {
    await dbConnect();
    const { id } = params;
    const deleted = await OutrosObjetos.findByIdAndDelete(id);
    if (!deleted) {
      return new Response(JSON.stringify({ error: "Registro não encontrado" }), { status: 404 });
    }
