// app/api/eletroeletronicos/[id]/route.js
...
export async function GET(request, { params }) {
  try {
    await dbConnect();
    const { id } = params;
    const record = await EletroEletronicos.findById(id);
    if (!record) {
...
export async function PUT(request, { params }) {
  try {
    ...
    const updatedRecord = await EletroEletronicos.findByIdAndUpdate(id, body, { new: true, runValidators: true });
    if (!updatedRecord) {
      return new Response(JSON.stringify({ error: "Registro não encontrado" }), { status: 404 });
    }
...
export async function DELETE(request, { params }) {
  try {
    await dbConnect();
    const { id } = params;
    const deleted = await EletroEletronicos.findByIdAndDelete(id);
    if (!deleted) {
      return new Response(JSON.stringify({ error: "Registro não encontrado" }), { status: 404 });
    }
