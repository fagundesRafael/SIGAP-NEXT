// components/SectionGeladeira.js
"use client";
...
export default function SectionGeladeira({ geladeiras, setGeladeiras }) {
  ...
  function handleAddMarca() {
    ...
    const existing = geladeiras.find(
      (item) => item.marca.toLowerCase() === marcaInput.trim().toLowerCase()
    );
    if (!existing) {
      setGeladeiras([...geladeiras, { marca: marcaInput.trim(), modelos: [] }]);
    }
  ...
  function handleAddModelo() {
    ...
    const index = geladeiras.findIndex(
      (item) => item.marca.toLowerCase() === marcaInput.trim().toLowerCase()
    );
    if (index === -1) {
    ...
    if (geladeiras[index].modelos.includes(modeloInput.trim())) {
      setError("Modelo já existe para essa marca.");
      return;
    }
    setError("");
    const updated = [...geladeiras];
    updated[index].modelos.push(modeloInput.trim());
    setGeladeiras(updated);
    setModeloInput("");
  }
  ...
  return (
    <div className="mb-2 p-4 rounded bg-c_deep_gray_black relative">
      <div className="flex items-center gap-2">
        <input
          type="text"
          placeholder="Inserir marca da geladeira"
          value={marcaInput}
      ...
      {showModeloInput && (
        <div className="flex items-center gap-2 mt-2">
          <input
            type="text"
            placeholder="Inserir modelo da geladeira"
            value={modeloInput}
      ...
      <div className="mt-2 text-[11px]">
        <h3 className="font-bold">Marcas e Modelos Registrados (Geladeiras):</h3>
        {geladeiras.length === 0 ? (
          <p>Nenhuma marca registrada.</p>
        ) : (
          <ul>
            {geladeiras.map((item, idx) => (
