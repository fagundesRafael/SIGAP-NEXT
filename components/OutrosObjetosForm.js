// components/OutrosObjetosForm.js
"use client";
...
// Para "Outros Objetos", o tipo é fixo
const TIPO_FIXO = "Outro Objeto";

export default function OutrosObjetosForm({ initialData = {}, onSubmit, isUpdating = false, title }) {
  const [formData, setFormData] = useState({
    ...
    unidMedida: "unid",
    marca: "",
    // Não há campo "modelo" para Outros Objetos
    cor: "",
    status: "",
    destino: "",
    secao: "",
  ...
  // Carregar as configurações para obter as opções de marca (a partir do array "outrosobjetos" no config)
  ...
  // Disponíveis para marca vêm de configs.outrosobjetos (array de strings)
  const availableMarcas = configs ? (configs.outrosobjetos || []) : [];
    ...
    <div className="min-h-screen text-white bg-c_deep_black p-1 rounded-md border border-gray-500 shadow">
      ...
      <form onSubmit={handleSubmit} className="flex justify-between p-4 text-xs">
        ...
        <div className="flex flex-col gap-4 w-[45%]">
          ...
          {/* Tipo fixo e customTipo */}
          <div>
            <label className="block font-medium">Tipo:</label>
            <div className="flex items-center gap-2 bg-c_deep_gray_black p-2 rounded-md">
              <input
                type="radio"
                name="tipo"
                value={TIPO_FIXO}
                checked={true}
                readOnly
                className="w-2.5 h-2.5 border-2 border-gray-400 rounded-full bg-green-600"
              />
              <span>{TIPO_FIXO}</span>
            </div>
            <div className="mt-2">
              ...
              <input
                ...
                placeholder="Informe detalhes do objeto"
              />
            </div>
          </div>
          ...
          {/* Campo para Marca (não há Modelo) e Data */}
          <div className="flex flex-wrap justify-between">
            <div>
