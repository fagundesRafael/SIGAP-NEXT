// components/SectionOutroEletroEletronico.js
"use client";
...
export default function SectionOutroEletroEletronico({ outroeletroeletronicos, setOutroeletroeletronicos }) {
  ...
  function handleAddMarca() {
    ...
    const existing = outroeletroeletronicos.find(
      (item) => item.marca.toLowerCase() === marcaInput.trim().toLowerCase()
    );
    if (!existing) {
      setOutroeletroeletronicos([...outroeletroeletronicos, { marca: marcaInput.trim(), modelos: [] }]);
    }
  ...
  function handleAddModelo() {
    ...
    const index = outroeletroeletronicos.findIndex(
      (item) => item.marca.toLowerCase() === marcaInput.trim().toLowerCase()
    );
    if (index === -1) {
    ...
    if (outroeletroeletronicos[index].modelos.includes(modeloInput.trim())) {
      setError("Modelo já existe para essa marca.");
      return;
    }
    setError("");
    const updated = [...outroeletroeletronicos];
    updated[index].modelos.push(modeloInput.trim());
    setOutroeletroeletronicos(updated);
    setModeloInput("");
  }
  ...
  function handleConfirmDelete() {
    ...
    if (deleteModalData.type === "brand") {
      const updated = outroeletroeletronicos.filter(
        (item) => item.marca !== deleteModalData.brand
      );
      setOutroeletroeletronicos(updated);
    } else if (deleteModalData.type === "model") {
      const updated = outroeletroeletronicos.map((item) => {
  ...
  return (
    <div className="mb-2 p-4 rounded bg-c_deep_gray_black relative">
      <div className="flex items-center gap-2">
        <input
          type="text"
          placeholder="Inserir marca do outro eletroeletrônico"
          value={marcaInput}
      ...
      {showModeloInput && (
        <div className="flex items-center gap-2 mt-2">
          <input
            type="text"
            placeholder="Inserir modelo do outro eletroeletrônico"
            value={modeloInput}
      ...
      <div className="mt-2 text-[11px]">
        <h3 className="font-bold">Marcas e Modelos Registrados (Outros Eletroeletrônicos):</h3>
        {outroeletroeletronicos.length === 0 ? (
          <p>Nenhuma marca registrada.</p>
        ) : (
          <ul>
            {outroeletroeletronicos.map((item, idx) => (
